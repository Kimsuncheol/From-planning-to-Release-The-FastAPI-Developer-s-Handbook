...
from sqlmodal import SQLModel, Field, Relationship, Text, JSON, func, String, Column
...
class Calendar(SQLModel, table=True):
    ...
    topics: list[str] = Field(
        ...
        description="Topics to share with the guest"
        )
    description: str = Field(sa_type=Text, description="Description to show to the guests")
...
class TimeSlot(SQLModel, table=True):
    ...
    weekdays: list[int] = Field(
        sa_type=JSON().with_variant(JSONB(astext_type=Text()), "postgresql"), 
        description="Available weekdays for reservation"
    )
